...
String::String(const char* inData)
{
	...
	strncpy(contents, inData, size + 1);		// input_data is copied to the contents
}
...
String::String(const String& originalString)
{
	...
	strncpy(contents, originalString.contents, size + 1);	 // copy between two contents
}
...
void String::setContents(const char* inContents) {
	...
	contents = new char[size + 1];   // +1 for null character
	strncpy(contents, inContents, size + 1);		// inputContents is copied to the contents
}
